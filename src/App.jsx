...
function App() {
  ...
  function adcionarTarefa() {
    tarefa
      ? setList([...list, { id: uuidv4(), tarefa, concluida: false }])
      : alert("Escreva uma tarefa antes de adcionar");
  }

  function buscarTarefas() {
    const tarefasFiltradas = list.filter((item) =>
      item.tarefa.toLowerCase().includes(tarefa.toLowerCase())
    );
            ...
            list.map((item) => (
              ...
              >
                <li>
                  ...
                  {item.tarefa}{" "}
                  <Deletar onClick={() => deletarTarefa(item.id)} />{" "}
                </li>
              </ListItens>
            ))
          ) : (
